...
- **Enemies never reposition on their own — for now.** Soldiers only turn to face threats and fight on contact — no patrol, no chase — with one deliberate exception already shipped (a melee soldier answering a Rifleman's call for help: `Soldier.moveToward`, used only from `GameScene.updateSoldierTargeting`). **This was chosen for implementation simplicity in the current build, not as a permanent design pillar.** An idea that needs enemies to reposition isn't disqualified by this rule — it's a candidate to deprioritize until the foundation (level authoring, ranged-enemy tuning, the trail/movement systems) can support it without a bigger rework, not a mechanic to reject outright. See "Working conventions" below for how to weigh a new idea against a rule like this one.
...
- **The invariants documented here and in docs/FIRST_BUILD.md are this project's current decisions, not permanent laws.** Several — "enemies never reposition on their own" is the clearest example — were chosen for implementation simplicity in an early build, not as design pillars. When a new idea conflicts with one, don't reject it just because it contradicts existing text; work out which of these actually applies, and say which one explicitly:
  - **Deprioritize** — the idea is fine, it just needs foundation this project doesn't have yet (the common case for anything touching enemy repositioning).
